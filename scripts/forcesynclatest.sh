...
forcesync() {
  ...
  blockhash=`curl -s https://explorer.uzlocoin.io/api/getblockhash?height=$blockcount`
  echo "| ଓ hash : $blockhash"
  curl -s https://explorer.uzlocoin.io/block/$blockhash > /dev/null
  echo "╘═══════════════════════════════>>"
}
...
main() {
  ...
  currentblockcount=`curl -s https://explorer.uzlocoin.io/api/getblockcount`
